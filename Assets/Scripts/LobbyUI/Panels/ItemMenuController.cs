...
public class ItemMenuController : PanelController
{
    ...
    public void Load(int type)
    {
        ...
        InfoMenu.Load();
        tabType = type; 
        string name = type == 0 ? "GridUnit_Item_ETC" : "GridUnit_Item_Consume";

        GameObject gridUnitPrefab = UIManager.instance.GetGridUnitPrefab(name);
        if(gridUnitPrefab != null)
        {
            PlayerItemData inventory = (type == 0) ? UIDataProcess.GetETCItemInventory().ItemList : UIDataProcess.GetConsumeItemInventory().ItemList;

            foreach (var item in inventory)
            {
                var itemInfo = UIDataProcess.GetItemInfo(item.Value.iItemIndex);
                if(itemInfo == null)
                {
